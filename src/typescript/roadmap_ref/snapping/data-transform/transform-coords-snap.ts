...
export  function transformSnapCoordsInAbsolute(parentNodeId:string, snappingsCoords: ISnapDrawCoordintes[]) {
    const offset = getNodeAbsoluteCoordsCenter(parentNodeId);
    const transformedSnappings = snappingsCoords.map((snapping) => {
        return {
