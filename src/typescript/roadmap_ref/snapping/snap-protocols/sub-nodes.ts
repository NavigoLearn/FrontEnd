...
export function snapSubNodeProtocol(
    ...
    const parentId = getNodeByIdRoadmapSelector(draggedNodeId).properties.nestedWithin;

    const externalAnchors = getSubNodeExternalAnchorsPositions(draggedNodeId, [draggedNodeId]);
    ...
    const adjustedSnappingLinesCoords = transformSnapCoordsInAbsolute(parentId, snappingLinesCoords);
